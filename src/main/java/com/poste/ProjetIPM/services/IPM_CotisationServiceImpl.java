...
public class IPM_CotisationServiceImpl implements IPM_CotisationService {
    ...
    private boolean readDataFromExcel(MultipartFile file) {
        if (ipm_cotisationRepository.findByFileName(FilenameUtils.getName(file.getOriginalFilename()))!=null) {
            return false;
        } else {
...
//                       FilenameUtils.getName(file.getOriginalFilename()), new Date()));
                    ipm_cotisationRepository.save(new IPM_Cotisation(row[0],row[1],
                            FilenameUtils.getName(file.getOriginalFilename()),new Date()));
    ...
    private boolean readDataFromJson(MultipartFile file) {
        try {
            ...
            List<IPM_Cotisation> ipmCotisations= Arrays.asList(mapper.readValue(inputStream, IPM_Cotisation[].class));
            if(ipmCotisations!=null && ipmCotisations.size()>0){
                for(IPM_Cotisation  ipm_cotisation : ipmCotisations){
